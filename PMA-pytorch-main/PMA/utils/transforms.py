...
from classification_ModelNet40.utils.randomn import uniform_2_sphere
import classification_ModelNet40.utils.se3 as se3
import classification_ModelNet40.utils.so3 as so3


class SplitSourceRef:
    """Clones the point cloud into separate source and reference point clouds"""
    def __call__(self, sample: Dict):
        sample['points_src'] = sample.pop('lr')
        sample['points_ref'] = sample.pop('gt')
        if isinstance(sample['points_src'], torch.Tensor):
            sample['points_src'] = sample['points_src'].detach()
            sample['points_ref'] = sample['points_ref'].detach()
        else:  # is numpy
            sample['points_src'] = sample['points_src'].copy()
            sample['points_ref'] = sample['points_ref'].copy()
...
class Resampler:
    ...
    def __call__(self, sample):
        ...
        if 'gt' in sample and 'lr' in sample:
            sample['gt'] = self._resample(sample['gt'], self.num)
            sample['lr'] = self._resample(sample['lr'], self.num)
        else:
...
class RandomJitter:
    ...
    def __call__(self, sample):
        ...
        if 'gt' in sample and 'lr' in sample:
            sample['gt'] = self.jitter(sample['gt'])
            sample['lr'] = self.jitter(sample['lr'])
        else:
...
class RandomTransformSE3:
    ...
    def __call__(self, sample):
        ...
        if 'gt' in sample and 'lr' in sample:
            sample['gt'], _, _ = self.transform(sample['gt'])
            sample['lr'], _, _ = self.transform(sample['lr'])
        else:
...
class ShufflePoints:
    ...
    def __call__(self, sample):
        if 'gt' in sample and 'lr' in  sample:
            sample['gt'] = np.random.permutation(sample['gt'])
            sample['lr'] = np.random.permutation(sample['lr'])
        else:
...
class Dict2PointnetLKList:
    ...
    def __call__(self, sample):
        ...
        if 'gt' in sample and 'lr' in sample:
            ...
            return sample['gt'][:, :3], sample['lr'][:, :3]
        else:
            # Train PointNetLK
